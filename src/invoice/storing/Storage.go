...
func CreateWithUuid (instance model.Invoice, response *model.Response){
  if response.Code > -4{
...
func create (uuid string, instance model.Invoice, response *model.Response){
	bucket := initialize()
...
func Remove(key string, response *model.Response){
	bucket := initialize()
	var cas gocb.Cas
...
func MutateWithUuid (payment model.Payment, response *model.Response){
	uuid_invoice, errUuid := uuid.NewRandom()
	if errUuid == nil {
...
func mutate(key string, payment model.Payment, response *model.Response){
	bucket := initialize()
...
func RetrieveInvoices(response *model.Response, invoices *model.InvoicesResponse){
	bucket := initialize()
	invoicesQuery := gocb.NewN1qlQuery("SELECT * FROM Invoices")
	rows, err := bucket.ExecuteN1qlQuery(invoicesQuery, nil)
	var row map[string]model.Invoice
	for rows.Next(&row) {
